...
def plot_aggregated_matrices_with_compartments(celltype_groups, bulk_pcas, bulk_flips, cpg_bins, out_dir):
    ...
    for celltype, files in celltype_groups.items():
        ...
        # Determine matrix size for figure scaling
        n_bins = bulk_Q_plot_size = cpg_bins.shape[0]

        # Figure: square heatmap on top, narrow compartment strip below, x-axes aligned
        # We fix the heatmap to be square by controlling figure width from the axes size
        heatmap_size = 8       # inches: both width and height of the heatmap axes
        colorbar_pad = 1.2     # inches reserved for colorbar
        bottom_height = 2.0    # inches for compartment score strip
        fig_width = heatmap_size + colorbar_pad
        fig_height = heatmap_size + bottom_height + 0.6  # 0.6 for hspace/title

        fig = plt.figure(figsize=(fig_width, fig_height))

        # Use GridSpec with precise height ratios and no hspace so x-axes align
        gs = GridSpec(2, 1,
                      height_ratios=[heatmap_size, bottom_height],
                      hspace=0.08,
                      left=0.08, right=(heatmap_size / fig_width) + 0.04,
                      top=0.95, bottom=0.07)

        # Top: Contact matrix heatmap
        ax1 = fig.add_subplot(gs[0])
        bulk_Q_plot = bulk_Q.copy()
        np.fill_diagonal(bulk_Q_plot, np.nan)
        vmax = np.nanpercentile(bulk_Q_plot, 98)
        im = ax1.imshow(bulk_Q_plot, cmap='Reds', aspect='equal', interpolation='none',
                        vmin=0, vmax=vmax,
                        extent=[0, n_bins, n_bins, 0])  # align image to bin coordinates
        ax1.set_title(f'Aggregated Contact Matrix - {celltype}', pad=8)
        ax1.set_ylabel('Bin ID')
        ax1.set_xlim(0, n_bins)
        ax1.set_xticklabels([])   # hide x tick labels on top panel
        ax1.grid(False)
        ax1.set_facecolor('white')
        for spine in ax1.spines.values():
            spine.set_visible(False)

        # Colorbar placed to the right of the heatmap axes only
        cbar_ax = fig.add_axes([
            (heatmap_size / fig_width) + 0.05,  # left
            gs[0].get_position(fig).y0,          # bottom (matches heatmap bottom)
            0.03,                                 # width
            gs[0].get_position(fig).height        # height (matches heatmap height)
        ])
        fig.colorbar(im, cax=cbar_ax, label='Raw Contacts')

        # Bottom: Filled-area compartment score plot
        ax2 = fig.add_subplot(gs[1])
        bins = np.arange(n_bins)
        zeros = np.zeros_like(pc1_plot)

        ax2.fill_between(bins, pc1_plot, zeros,
                         where=(pc1_plot >= 0), interpolate=True,
                         color='#E07070', alpha=0.85, label='A (active)')
        ax2.fill_between(bins, pc1_plot, zeros,
                         where=(pc1_plot < 0), interpolate=True,
                         color='#6F80C8', alpha=0.85, label='B (repressive)')
        ax2.plot(bins, pc1_plot, color='#3B3B8F', linewidth=0.9)
        ax2.axhline(y=0, color='gray', linestyle='--', linewidth=0.8, alpha=0.7)

        ax2.set_xlim(0, n_bins)   # same x range as heatmap → vertical alignment
        ax2.set_ylim(-2.5, 2.5)
        ax2.set_xlabel('Bin ID')
        ax2.set_ylabel('Compartment Score')
        ax2.set_title('Bulk Compartment Scores (Quantile-normalized for visualization)', pad=4)
        ax2.legend(loc='upper left', fontsize=9, framealpha=0.9)
        ax2.grid(False)
        ax2.set_facecolor('white')

        plt.savefig(os.path.join(out_dir, f'aggregated_matrix_{celltype}.pdf'),
                    format='pdf', bbox_inches='tight')
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f'new_aggregated_matrix_{celltype}.png'), dpi=300, bbox_inches='tight')
        plt.close()
