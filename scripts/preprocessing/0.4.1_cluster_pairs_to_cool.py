...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 24 00:35:02 2025
...
def load_and_filter_hic_data(filepath):
    ...
    df = df.dropna(subset=['pos1', 'pos2', 'strand1', 'strand2', 'barcode1'])
    df = df.drop_duplicates(subset='readID')
    return df[['chrom1', 'pos1', 'chrom2', 'pos2', 'barcode1']], chrom_sizes
...
def run_cluster_pipeline(input_dir, chrom, bin_size, output_dir):
    ...
    if len(pairs_files) != 20:
        print(f"Warning: Found {len(pairs_files)} .pairs files instead of 20.")
...
if __name__ == "__main__":
    run_cluster_pipeline(
        input_dir="pairs_clusters",         # Input directory with .pairs files
        chrom="chr1",                       # Target chromosome
        bin_size=500000,                    # Bin size (500 kb)
        output_dir="accurate_cool_clusters" # Output directory for .cool files
    )